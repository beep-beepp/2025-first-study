...
3. 주요 키워드
    ...
    2. synchronized, Lock
       ...
       - 락 해제시 메모리에 즉시 반영하고, 다른 스레드가 락 획득시 메모리에서 최신 값을 읽는다.
         - 락 해제시, 해당 스레드가 그동안 작업한 값을 메모리에 flush하도록 보장한다.
         - 락 획득시, 메모리에서 최신 값을 읽어와 자신의 작업 메모리(캐시)에 반영한다.
       - 가시성, 원자성, 순서성 모두 보장한다.
    3. happens-before  
       - "한 작업의 결과가 다른 작업에서 반드시 보인다."는 순서보장 관계를 말한다.
       - A happens-before B : A가 끝난 후에 모든 결과가 B에서 반드시 보인다.
